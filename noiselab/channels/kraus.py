...
class KrausChannel(QuantumChannel):
    ...
    def apply(self, rho: DensityMatrix) -> DensityMatrix:
        """
        Применить канал к квантовому состоянию

        ε(ρ) = Σᵢ Kᵢ ρ Kᵢ†

        === Процесс применения ===
        1. Для каждого оператора Kᵢ вычисляем Kᵢ ρ Kᵢ†
           - Это "сжатие" с обеих сторон: Kᵢ действует слева, Kᵢ† справа
           - Физически: эволюция, соответствующая исходу i

        2. Суммируем все вклады: ε(ρ) = Σᵢ Kᵢ ρ Kᵢ†
           - Усреднение по всем возможным исходам
           - Получаем смешанное состояние, даже если входное было чистым
        Args:
            rho: Входная матрица плотности

        Returns:
            Выходная матрица плотности ε(ρ)
        """
        ...
        # Применяем операторы Крауса: ε(ρ) = Σᵢ Kᵢ ρ Kᵢ†
    ...
    def kraus_rank(self) -> int:
        """
        Ранг Крауса - минимальное число операторов Крауса, необходимых для представления канала

        === Физический смысл ===
        Характеризует "сложность" канала:
        - Ранг 1: унитарная эволюция (обратимый процесс)
        - Ранг 2-4: простой шум (depolarizing, bit flip, etc.)
        - Ранг > 4: сложный шум или композиция нескольких каналов

        === Вычисление ===
        1. Строим Choi matrix J
        2. Диагонализуем: J = Σᵢ λᵢ |vᵢ⟩⟨vᵢ|
        3. Считаем ненулевые λᵢ (с порогом 1e-10 для численной стабильности)

        Returns:
            Ранг Крауса (минимальное число операторов)
        """
